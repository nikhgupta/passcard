RSpec.describe Passe::Outputter do
  ...
  class DummyOutputter < Passe::Outputter
    ROW_HEADERS = "☾ ✈ ☎ ☀ ☁ ☂ ☃ ★ ⌘ ♞ "
    COLUMN_HEADERS = "♞ ⌘ ★ ☃ ☂ ☁ ☀ ☎ ✈ ☾ "
  end

  it "has access to Passe::Generator instance" do
  ...
  it "allows registering methods in subclasses as outputters" do
    ...
    expect(Passe.outputters).to have_key(:to_a)
    expect(Passe.outputters[:to_a]).to eq [klass, :to_a]

    klass = Class.new(described_class){ register to_a: :create_a }
    expect(Passe.outputters).to have_key(:to_a)
    expect(Passe.outputters[:to_a]).to eq [klass, :create_a]

    klass = Class.new(described_class){ register 'to_c', to_d: :create_d }
    expect(Passe.outputters).to have_key(:to_c)
    expect(Passe.outputters).to have_key(:to_d)
    expect(Passe.outputters[:to_c]).to eq [klass, :to_c]
    expect(Passe.outputters[:to_d]).to eq [klass, :create_d]

    expect {
      klass = Class.new(described_class){ register }
    }.to raise_error(Passe::Error)
    expect {
      klass = Class.new(described_class){ register({}) }
    }.to raise_error(Passe::Error)
  end
  ...
  describe "#get_grid" do
    ...
    it "resets the grid on each use as per given options" do
      ...
      expect(subject.grid).to be_a_passe_grid.with_size(10, 20)

      subject.get_grid type: :alphanum
      expect(subject.grid).to be_a_passe_grid.with_size(26, 26)
    end

    it "allows fetching a random subgrid" do
      subject.get_grid type: :random
      expect(subject.grid).to be_a_passe_grid.with_size(20, 30)
    ...
    it "allows fetching two distinct numbers only Pincard subgrids" do
      subject.get_grid type: :pincard
      expect(subject.grid).to be_a_passe_grid.with_size(10, 20).numeric
      ...
      expect(subject.grid).not_to eq grid
      expect(subject.grid).to eq(alternate).and be_a_passe_grid
        .with_size(10, 20).numeric
    end

    it "allows fetching two distinct alphanumeric subgrids" do
      subject.get_grid type: :alphanum
      expect(subject.grid).to be_a_passe_grid.with_size(26, 26).alphanumeric
    ...
    it "allows fetching a square subgrid of alphabets, numbers and symbols" do
      ...
      expect(subject.grid).to be_a_passe_grid.with_size(20, 20).with_symbols
    ...
    it "allows fetching a large card size subgrid of alphabets, numbers and symbols" do
      ...
      expect(subject.grid).to be_a_passe_grid.with_size(15, 30).with_symbols
    ...
    it "allows fetching card size subgrid of alphabets, numbers and symbols" do
      ...
      expect(subject.grid).to be_a_passe_grid.with_size(10, 20).with_symbols
    end

    it "defaults to returning the complete grid" do
      expect(subject.get_grid).to eq(reader.grid).and be_a_passe_grid.with_size(80, 80)
    end
  end
  ...
  it "does not generate the grid on instantiation" do
    ...
    subject.get_grid
    expect(subject.grid).to be_a_passe_grid
  end
  ...
  it "has access to Passe::Palette when required" do
    expect(subject.palette.count).to eq 0
    subject.get_grid
    expect(subject.palette).to be_a(Passe::Palette)
