RSpec.describe Passe::Reader do
  let(:key) { "secret-word" }
  let(:enc_text) { Passe::Generator.new(key).run }
  let(:reader){ Passe::Reader.new(key, enc_text) }

  it "allows easy access to Passe options that were used to generate" do
  ...
  it "allows easy access to Passe grid" do
    expect(reader.grid).to be_a(Passe::Grid)
    expect(reader.grid.size).to eq reader.opts["size"]
  end

  context "extracting sub-grids" do
    it "pre-defined numeric sub-grid" do
      expect(reader.numeric_grid).to be_a(Passe::Grid)
      expect(reader.numeric_grid.size).to eq reader.opts["numeric"]
    end

    it "allows easy access to alphanumeric sub-grid in the Passe grid" do
      expect(reader.alpha_grid).to be_a(Passe::Grid)
      expect(reader.alpha_grid.size).to eq reader.opts["alpha"]
    end

    it "allows fetching a random sub-grid" do
      expect(reader.random_grid).to be_a_passe_grid.with_size(10, 10)

      expect(reader.random_grid(5, 5)).to be_a_passe_grid.with_size(5, 5)
      expect(reader.random_grid(100, 100)).to be_a_passe_grid.with_size(*reader["size"])

      expect(reader.random_grid(0, 0)).to be_a_passe_grid.with_size(0, 0)
      expect(reader.random_grid(0, 1)).to be_a_passe_grid.with_size(0, 0)
      expect(reader.random_grid(1, 0)).to be_a_passe_grid.with_size(0, 0)
      expect(reader.random_grid(1, 1)).to be_a_passe_grid.with_size(1, 1)
    end
  ...
  context "reg. outputters" do
    it "delegates method to outputter which register such a method" do
      MP32Outputter = Class.new(Passe::Outputter) do
        register :to_mp3
      end
  ...
  context "#to_s" do
    it "delegates #to_s to the AsciiOutputter" do
      double = double()
      expect_any_instance_of(Passe::AsciiOutputter).to receive(:to_ascii)
        .and_return(double)
      expect(reader.to_s).to eq double
    end

    it "uses registered outputter for #to_s if available" do
      Class.new(Passe::Outputter){ register :to_s; def to_s; "a"; end }
      expect(reader.to_s).to eq "a"
    end

    it "fallsback to #to_s for the grid when outputter #to_s is nil" do
      # Passe.unregister_outputter :to_s
      HELPER = Class.new(Passe::Outputter){ register :to_s; def to_s; end }
